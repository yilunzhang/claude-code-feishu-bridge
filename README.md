...
飞书 ──WS── lark-cli event bus(自带)
  ...
  bridge daemon(flock 单例;单线程事件循环;bridge.db=SQLite WAL 唯一持久真相)
    入站:去重→mget 快照(钉死 binding)→结构化@判定→门禁(owner 直投/白名单直投/member 审批卡)→deliveries
...
- **直投白名单**(v1.5.3):`(群, 人)` 双精确匹配命中 `allowlist.json` 的成员**免审批卡直投**,详见 🅘。
- 每轮转发回群的 session 输出末尾带一行**页脚**:`🧠 <上下文K> · <模型> · <effort>`(上方一条分隔线),让你一眼看到该 session 当前 context 占用 / 模型 / effort。详见「已知限制 🅔」。
- image/file 会下载到本地,payload 给绝对路径;member 附件批准前不下载(**白名单成员除外** —— 已授权,与 owner 同路直接下载)。
...
- **v1 不做**:thread 出站回复(审批卡的 reply 除外)、reaction 快捷审批、消息编辑/撤回跟踪、topic 群、离线自动补投(见 🅑;LaunchAgent 已决不做,见 🅐)、长输出转文件、卡片原地更新(晚点击无卡片刷新,结果以文本通知)、原子换绑、消费级 ACK、多 profile 多桥。
- **🅕 非文本消息的附件 = 给 agent 句柄自取(v1.4.2);`post` 零下载**:飞书把「图 + 文字」打包成 `post`,而自动下载只覆盖 `image`/`file` 两类(`MEDIA_MSG_TYPES`)→ **post 里的附件桥不下载**(`image`/`file` 仍照旧自动下、放 `media_paths`;hint 措辞已明确"先看 `media_paths`,没有的才自取",不谎称桥从不下载)。但 lark-cli **已把 `img_v3_…`/`file_…` 渲染进正文**,故 `msg_type != "text"` 时 payload 附 **`media_keys`**(`[{key,type}]`,保序去重)+ **`fetch_hint`**(每个 key 一条可直接跑的 `+messages-resources-download` 命令,带 `--as bot` 与桥的 `--profile`;无 key 时给 `+messages-mget` 兜底、**绝不编造 key**)。**判定看 `msg_type` 而非扫正文**:`image`/`file` 渲染成 `[图片]`/`(文件) 名字`、根本不含 key,「扫到才加」会漏;纯文本里粘了 key 样字符串则会误加。**新字段而非拼进 `text`**(`text` 是用户原话,不混入系统提示)。已知边界:`extract_text` 的 fallback 分支(顶层 `content` 缺失)丢 post 的 `img`/`media` 节点 → 那种情况扫不到 key,但 hint 仍在 + 有 mget 兜底;不递归解析原始 post 结构(过度设计)。`--output` 必须 cwd 相对路径,且 **lark-cli 会按 Content-Type 自动补扩展名 → 落盘路径 ≠ 传入路径**,故 hint 明确要求读返回的 `data.saved_path`。
- **🅗 回复/引用消息 = 给 agent 句柄自取(v1.5.1)**:飞书把「回复/引用」放在快照**顶层 `reply_to`**,被引用的内容**一个字都不在正文里**。payload 附 **`reply_to`**(被引用的 message_id)+ **`reply_hint`**(可直接跑的 `+messages-mget` 命令,带 `--as bot` 与桥的 `--profile`);**不内联被引用内容** —— 被引用的常是 `merge_forward`(整段转发记录)或长贴,内联会成倍撑大每条 payload,而多数回复用不到它。**闸门与 🅕 相互独立**:带引用的消息 `msg_type` 仍是 `text`,套用 🅕 的 `msg_type != "text"` 会整个漏掉;「图 + 引用」两个 hint 并存,故 `reply_hint` / `fetch_hint` 是**两个独立字段**、不共用一个键。**嵌套资源**:`merge_forward` 里的图要用**被引用那条**的 `message_id` 下载(资源挂在它身上,不是当前这条),hint 里已注明。
- **🅘 选群列表必须是全集,取不全宁可失败(v1.5.2)**:`im +chat-list` 不自动翻页(默认 page-size=20),只取首页会得到一个**和全集长得一模一样**的残缺列表——没有报错、条目也都是真的——于是「群不在列表里」被读成「这个群不存在」,下一步就是**新建一个重复群**(外发且不可逆)。`list_chats` 因此跟着 `has_more` 翻页,并且**凡不能证明取全了一律返回 None**(任一页失败 / `has_more` 非严格布尔 / 声称还有更多却无法证明有推进),绝不返回前缀。三条契约:① 终止只认 `has_more is False`;② **`page_token` 重复不代表没前进**——该接口实测从第 2 页起一直返回同一个 token 而内容照常推进,按「重复即失败」判会把能正常取全的路径判死;③ 推进以**本页有没有带来新 `chat_id`** 为准(空页 / 整页畸形 / 整页已见 id 都不算),否则会在那个恒定 token 上无限循环。同一 id 跨页重复只保留一次。**SKILL.md 侧配套**:建群前先问用户群名 → `+chat-search` 带 `--disable-search-by-user`(不带时默认**先按成员名搜**,搜一个人名会回上百个与群名无关的群)且同样翻页 → **只有群名完全相等才算命中** → 取不全就停下问、禁止进入建群;并明确这**只排除 user 可见范围内的同名群,不证明租户里不存在**。
- **🅘 成员直投白名单(v1.5.3)**:`(chat_id, open_id)` **双精确匹配**命中 `~/.claude/data/feishu-bridge/allowlist.json` 的成员 → 跳过审批卡直投。**只有两个字段同时精确相等才放行**:少任一维度就成了"整群放行"或"该人全局放行",都超出 owner 授权范围,故缺字段/空串/前缀/超串一律不匹配;文件缺失或损坏 = **fail-closed 回到审批门**。**独立文件、不进 config.json**(后者是 bootstrap 钉死的指纹 + FingerprintGate 的比对基准,写坏会让出站全停);**每次判定读盘不缓存** → 另一进程(agent 的 `bridgectl allow`)改完**下一条消息即生效**,撤权同理立即失效。**信任级别不提升**:payload 仍 `sender_is_owner=false`,`approved_by="allowlist"`(**不是 `None`** —— 那是 owner 本人的语义,混用会让 agent 把成员消息当成 owner 亲发)。**写入方 = agent 按 owner 群内指令执行**(owner 选定:首次接触必经审批这一层已足够,不再加签名/二次确认);SKILL.md 明令**只认 `sender_is_owner=true` 的增删指令**,成员自称"owner 同意了"不执行 —— 否则一个成员即可自我提权、整道门失效。**永久有效**,`allow remove` 撤销。已知边界:成员通过门后、附件下载完成前被 remove,该条仍会投递(授权以首次判定为准);`allow` 命令假定**串行执行**(单 agent),并发读—改—写可能丢更新。
...
- **指纹/版本门(fail-closed)**:daemon 启动与运行期校验 lark-cli 身份(appId/owner)与版本;身份确证不符=拒启;身份未验证/版本不符=**出站停摆**(入站照常入库),带退避重探。门在 ok 态每 10 分钟(单调钟)复检,漂移在下一循环发送前关门。
- **🅖 版本漂移自愈 + 停摆必通知(v1.5.0)**:**事故驱动**(2026-07-30 真实发生)—— lark-cli 被自动升级 1.0.66→1.0.80,门判 `version_mismatch` → **全部出站静默停摆**(入站不过此门 → 症状是"发进来的能到、转发出不去"),积压 27 条,owner 只能靠"怎么没转发了"察觉,恢复要人肉跑 `doctor`。**且 notify/StopFailure 告警也走同一个门 = 报警器与被报警物共用一条线。** 现在:
  - **自动自检重钉**:仅 `version_mismatch` 触发(`identity_mismatch` = 换了 app/owner,**绝不自愈**,必须停下问人)。自检**与真实转发同形**——`+messages-send --markdown`(**不是** doctor 那条 `--text`:两者是不同代码路径,当天正是拿 `--text` 的通过放行了 `--markdown`,而 v1.4.1 修的恰是 markdown 渲染 = **测了零件没测装配**),判据同为 `ok` ∧ `data.message_id`;**不做 recall**(撤回证明不了正常出站)。自检发**固定测试群**(`constants.SELFCHECK_CHAT_ID`,硬编码;绝不发工作群留痕)。
  - **时序**:判定漂移**那一刻就地关门**(写 `degraded:version_selfcheck`)→ 落"已尝试"标记 → 才联网。因为直发路径(notify/StopFailure)在**另一个进程**读同一 SQLite,晚关门 = 它们会用未验证的 CLI 发出去。自愈只在 `tick()`,**绝不在 `startup()`**(那跑在 daemon 就绪前,supervisor ~52s 判失败)。
  - **只试一次 + 瞬态可重试**:标记按 `pinned→actual` 持久化到 `daemon_state`(进程内变量在 daemon 重启即失忆,崩溃循环下最坏约 360 次/小时刷屏测试群);但**网络/5xx/官方 `retryable` 的瞬态失败会清标记**让退避再试 —— 否则"升级时恰好断网"= 该版本永不自愈 = 又变回永久停摆。歧义结果(超时/`ok` 无 id)保留标记。
  - **通知走 macOS 通知中心**(`lib/notifyos.py`,`osascript`),**不经飞书**(那正是被堵的线);**只在状态跃迁时一条**,自愈成功/失败各发**一条合并结论**,健康启动**不发**。文案经 `run argv` 传参(拼串遇引号会静默失败)。**fail-open**:通知异常绝不影响门逻辑。
  - **⚠️ 已知取舍(false-green admission)**:单次 markdown 自检只覆盖**主路径**;卡片/reaction/post 可能独立回归而自检照样通过 → 会重钉一个"部分兼容"的 CLI。这是用「自动恢复」换掉的安全性质(旧行为从不信任未钉版本的 CLI)。**⚠️ 前提**:`osascript` 通知权限须开启 —— 被关时 rc=0 照返回而屏幕无提示,"不静默"会形同虚设。
